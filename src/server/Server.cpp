...
void	Server::sendMessage(Client& client, const std::string& target, const std::string& message)
{
	...
	if (target.empty())
		_handleReply(client, AReply::getReply(411, *this, client, context));
	else if (message.empty())
		_handleReply(client, AReply::getReply(412, *this, client, context));
	else if (targetFd > 0)
		_sendPrivate(client, targetFd, message);
	else if(_channelExists(target))
		_sendPublic(client, target, message);
	else
		_handleReply(client, AReply::getReply(401, *this, client, context));
}
